...
class AutomationPauseCard extends LitElement {
  ...
  async _snooze() {
    if (this._selected.length === 0 || this._loading || this._duration === 0) return;
    ...
    try {
      ...
      this._showToast(`Paused ${count} automation${count !== 1 ? "s" : ""} for ${durationText}`);
  ...
  getCardSize() {
    const paused = this._getPaused();
    return 4 + Object.keys(paused).length;
  }
