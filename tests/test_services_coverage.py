...
class TestAsyncPauseAutomations:
    ...
    async def test_failed_wake_keeps_active_pause_when_future_schedule_replaces_it(self) -> None:
        ...
        with (
            patch("custom_components.autosnooze.services.async_set_automation_state", side_effect=fail_wake),
            patch("custom_components.autosnooze.services.schedule_disable") as schedule_disable,
        ):
            await async_pause_automations(
                mock_hass,
                data,
...
class TestServiceHandlerContracts:
    ...
    @pytest.mark.asyncio
    async def test_pause_by_area_forwards_notification_trigger(self) -> None:
        """pause_by_area should forward notification trigger settings through the shared filter path."""
        mock_hass, data, handlers = self._register_handlers()
        pause_by_area_handler = handlers["pause_by_area"]
