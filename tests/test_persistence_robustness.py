...
class TestDeletedAutomationCleanup:
    ...
    @pytest.mark.asyncio
    async def test_due_scheduled_automation_restores_notify_on_resume_when_loaded_as_paused(
        self, mock_hass: MagicMock, mock_store: MagicMock
    ) -> None:
        """Test that due scheduled snoozes preserve notify_on_resume during restore."""
        from custom_components.autosnooze.coordinator import async_load_stored as _async_load_stored
        from custom_components.autosnooze.models import AutomationPauseData
        ...
        assert data.paused["automation.existing_scheduled"].notify_on_resume is True
